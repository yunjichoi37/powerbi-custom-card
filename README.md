...
하나 이상의 값을 카드 형태로 나열하고, 전년 대비 증감(YoY)을 화살표와 색상으로 표시할 수 있다.
...
- **데이터 구성 방식 두 가지**
  - Category를 지정하면 카테고리별로 카드가 하나씩 생성된다. (예: 국가별 무역수지)
  - Category 없이 Value에 여러 측정값을 넣으면 측정값별로 카드가 하나씩 생성된다. (예: 당년 무역수지/수입금액)
- **전년/전기 대비(YoY) 표시**: `▲`/`▼`와 증감률(%)을 최대 3x3의 그리드 안에서 원하는 위치에 표시할 수 있다. 증가/감소에 대한 색상을 지정할 수 있다.
- **한국어 숫자 단위 자동 표시**: 보고서 언어가 한국어일 때 천/만/억/조 단위로 자동 변환해준다. (Power BI 기본 3자리 대신 한국식 4자리 사용)
- **레이아웃**: 자동(반응형) 또는 고정 열 수, 카드 너비/높이, 자동/수동 패딩, 카드 간 좌우·상하 여백 조정
- **요소 배치**: 제목/값/YoY를 최대 3×3 그리드에서 행·열 단위로 자유롭게 배치
- **카드 스타일**: 배경색·투명도, 테두리 색상·두께·모서리 반경
- **글꼴 커스터마이징**: 제목/값/YoY 각각에 대해 글꼴, 크기, 굵게/기울임/밑줄, 색상 지정
- **선택 및 툴팁**: 카드를 클릭하면 다른 시각화와 교차적으로 필터링이 들어가며, 마우스 오버 시 상세 툴팁이 표시된다.
...
- **요소 배치**: 그리드 행/열 수, 제목·값·YoY 각각의 위치(행,열) 지정
- **전년 대비 (YoY)**: 표시 여부, 글꼴, 증가/감소 색상
...
- Node.js
- [powerbi-visuals-tools](https://www.npmjs.com/package/powerbi-visuals-tools) (`pbiviz`) - devDependency로 포함되어 있어 `npm install` 시 함께 설치됨
